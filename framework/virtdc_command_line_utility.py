...
def show_domain_info(vm_id):
	...
	for node, vm_dict in host_vm_dict.iteritems():
			...
			if vm_id == vmid:
				...
				print '%s%s' %(str('Current CPU:').ljust(30),value.current_cpu)
				print '%s%s' %(str('Maximum CPU:').ljust(30),value.max_cpu)
				print '%s%s' %(str('Current Memory:').ljust(30),value.current_memory)
				print '%s%s' %(str('Maximum Memory:').ljust(30),value.max_memory)
